...
final class ExposePathsSecurityFilterChainTest
{
	@Autowired
	...
	@DisplayName("Should forbid access to unexposed paths")
	void shouldRejectAccessToOtherPaths(String description, String path) throws Exception
	{
		mockMvc.perform(get(path))
			   .andExpect(status().isUnauthorized()) // Spring Security returns 401 by default for unauthenticated
			   .andExpect(result -> assertThat(result.getResponse().getContentAsString())
	...
	private static Stream<Arguments> restrictedPathScenarios()
	{
		return Stream.of(
				...
				arguments("Random non-exposed path", "/admin/config")
		);
	}
...
class SecurityConfiguration
{
	...
	@Bean
	public SecurityFilterChain securePaths(HttpSecurity http, JwtFilter filter) throws Exception
	{
